...
impl Scratch {
    ...
    /// must not collide), canonicalized because the server canonicalizes
    /// workspace roots and document paths (macOS `/var` → `/private/var`).
    pub(crate) fn new(tag: &str) -> Self {
        ...
        match dunce::canonicalize(&dir) {
            ...
            Err(e) => panic!("scratch {}: {e}", dir.display()),
        }
    }
}
